...
<!--        </el-page-header>-->
        <router-link class="a" to="/Myclass" ><span style="color: #A15ADB;margin-right: 10px;margin-left: 10px">已创建的班级 </span></router-link>
        <router-link class="a" to="/Myclass" ><span style="color: #A15ADB">/</span></router-link>
        <router-link class="a" to="/Over" ><span style="margin-left: 10px;">已创建的班级</span></router-link>
        <p v-show="show" @click="ctr">管理</p>
        ...
        <div id="uk">
            ...
            <el-button class="but2" >
                ...
                <sapn  class="rig" v-show="showctr" @click="out(item.id,item.cname,item.classroom)">退出</sapn>
            </el-button>
        </div>
        </div>
    </div>
...
<script>
    ...
    export default {
     ...
     methods: {
            ctr(){this.show=!this.show;
                  this.showctr=true},
            go(id){
                this.$router.push({path:'/DesM',query:{cid:id}})
            },
         out(id,name,classroom){
             let rep=new URLSearchParams();
             let a=window.localStorage;
             rep.append('id',a.getItem('uid'));
